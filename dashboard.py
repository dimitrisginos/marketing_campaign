...
# =============================================================
#  Campaign Type: ROI & Engagement
# =============================================================
st.subheader("Which Campaign Type delivers the best ROI?")
...
st.info(
    f"**★ Key Finding:** Best → **{q1.iloc[0]['Campaign_Type']}** "
...
# =============================================================
st.subheader("Which Audience Segment converts best at the lowest cost?")
...
# 3 columns with slight size reduction via height parameter
...
with col1:
    ...
    fig.update_layout(coloraxis_showscale=False, xaxis_title="",
                      yaxis_title="Conv Rate (%)", height=340)
    st.plotly_chart(fig, use_container_width=True)

with col2:
    # ── CHANGE 2: reversed — smallest cost on top, largest on bottom
    q4c = q4.sort_values("Avg_Acq_Cost", ascending=True)
    fig2 = px.bar(q4c, y="Target_Audience", x="Avg_Acq_Cost", orientation="h",
                  ...
                  category_orders={"Target_Audience": q4c["Target_Audience"].tolist()})
    fig2.update_traces(textposition="outside")
    fig2.update_layout(coloraxis_showscale=False, yaxis_title="",
                       xaxis_title="Acquisition Cost ($)", height=340,
...
with col3:
    ...
    fig3.update_layout(coloraxis_showscale=False, xaxis_title="",
                       yaxis_title="Engagement Score", yaxis_range=[0, 11], height=340)
    st.plotly_chart(fig3, use_container_width=True)

st.info(
    f"**★ Key Finding:** Best → **{q4.iloc[0]['Target_Audience']}** "
...
# =============================================================
#  Campaign Type × Audience Heatmap (ROI only — CHANGE 3)
# =============================================================
st.subheader("Which Campaign × Audience combo maximizes ROI?")
...
st.info(
    f"**★ Key Finding:** Best combo → **{best_combo[0]} + {best_combo[1]}** "
...
# =============================================================
#  Geographic Performance — CHANGE 4: combined map
# =============================================================
st.subheader("Which City performs best per Campaign Type?")
...
q7_avg["lat"] = q7_avg["Location"].map(lambda x: city_coords.get(x, {}).get("lat"))
q7_avg["lon"] = q7_avg["Location"].map(lambda x: city_coords.get(x, {}).get("lon"))

q7_campaign = dff.groupby(["Location", "Campaign_Type"])["ROI"].mean().round(2).unstack()
q7_campaign_str = q7_campaign.apply(
    lambda row: "<br>".join([f"{col}: {val:.2f}" for col, val in row.items()]), axis=1
).reset_index()
q7_campaign_str.columns = ["Location", "breakdown"]
q7_avg = q7_avg.merge(q7_campaign_str, on="Location")
...
fig_map = px.scatter_mapbox(
    ...
    hover_name="Location",
    hover_data={"Avg_ROI": ":.2f", "lat": False, "lon": False, "breakdown": True},
    size_max=40,
    zoom=3,
    title="Avg ROI by City (bubble size = ROI | hover for Campaign breakdown)",
    mapbox_style="carto-positron",
)
fig_map.update_layout(margin={"r": 0, "t": 40, "l": 0, "b": 0}, height=480)
st.plotly_chart(fig_map, use_container_width=True)

q7_avg_sorted = q7_avg.sort_values("Avg_ROI", ascending=True)
best_city  = q7_avg_sorted.iloc[-1]["Location"]
worst_city = q7_avg_sorted.iloc[0]["Location"]
st.info(
    f"**★ Key Finding:** Best city → **{best_city}** "
    f"(ROI {q7_avg_sorted.iloc[-1]['Avg_ROI']:.2f}) | "
    f"Worst → **{worst_city}** "
    f"(ROI {q7_avg_sorted.iloc[0]['Avg_ROI']:.2f})"
)
st.divider()
...
# =============================================================
st.subheader("What is the optimal Campaign Duration?")
...
with col1:
    ...
    fig.update_layout(title="Avg ROI by Duration", yaxis_title="Avg ROI",
                      yaxis_range=[0, q9["Avg_ROI"].max() * 1.2], height=340)
    st.plotly_chart(fig, use_container_width=True)
...
with col2:
    ...
    fig2.update_layout(title="Avg Clicks by Duration",
                       ...
                       xaxis_title="Duration (days)", yaxis_title="Avg Clicks", height=340)
    st.plotly_chart(fig2, use_container_width=True)

with col3:
    # ── CHANGE 2: reversed — smallest cost on top, largest on bottom
    q9c = q9.copy()
    q9c["dur_label"] = q9c["Duration_Days"].astype(str) + " days"
    q9c = q9c.sort_values("Avg_Cost", ascending=True)
    fig3 = px.bar(q9c, y="dur_label", x="Avg_Cost", orientation="h",
                  color="Avg_Cost", color_continuous_scale="RdYlGn_r",
                  text=q9c["Avg_Cost"].map("${:,.0f}".format),
                  title="Avg Acquisition Cost ($) by Duration",
                  category_orders={"dur_label": q9c["dur_label"].tolist()})
    fig3.update_traces(textposition="outside")
...
st.info(
    f"**★ Key Finding:** Sweet spot → **{int(best_d['Duration_Days'])} days** "
...
# =============================================================
#  Channel ROI vs Cost Efficiency
# =============================================================
st.subheader("Which Channel gives the best ROI at the lowest cost?")
...
with col1:
    ...
    fig.update_traces(textposition="outside")
    fig.update_layout(coloraxis_showscale=False, xaxis_title="",
                      yaxis_title="Avg ROI", height=340)
    st.plotly_chart(fig, use_container_width=True)

with col2:
    # ── CHANGE 2: reversed — smallest cost on top, largest on bottom
    q2c = q2.sort_values("Avg_Cost", ascending=True)
    fig2 = px.bar(q2c, y="Channel_Used", x="Avg_Cost", orientation="h",
                  ...
                  category_orders={"Channel_Used": q2c["Channel_Used"].tolist()})
    fig2.update_traces(textposition="outside")
    fig2.update_layout(coloraxis_showscale=False, yaxis_title="",
                       xaxis_title="Acquisition Cost ($)", height=340,
                       yaxis={"autorange": "reversed"})
...
with col3:
    # ── CHANGE 5: no text labels on bubbles, only on hover; x-axis reversed
    ...
    fig3.add_trace(go.Scatter(
        ...
        customdata=np.stack([q2["Channel_Used"], q2["Avg_Conv"]*100], axis=-1),
        hovertemplate=(
            "<b>%{customdata[0]}</b><br>"
            "Avg ROI: %{y:.2f}<br>"
            "Avg Cost: $%{x:,.0f}<br>"
            "Conv Rate: %{customdata[1]:.1f}%"
            "<extra></extra>"
        ),
        marker=dict(
    ...
    fig3.update_layout(
        title="ROI vs Cost  (bubble size = Conversion Rate)",
        xaxis_title="Avg Acquisition Cost ($)",
        yaxis_title="Avg ROI",
        xaxis={"autorange": "reversed"},         # x-axis reversed
        showlegend=False,
        height=340,
    )
    st.plotly_chart(fig3, use_container_width=True)
...
st.info(
    f"**★ Key Finding:** Best ROI → **{best_ch['Channel_Used']}** "
...
ROI {q7_avg_sorted.iloc[-1]['Avg_ROI']:.2f} — best geographic market
""")
